...
| 17 | **COUNT is the lever** | **ANSWERED - it is LIQUIDITY, 88% out of reach** | 84.2% of certain market-seconds have no ask our side and 140,989 of 140,990 have one on the LOSING side. Only 39 of 333 silent closes are reachable by any constant. RESULTS_count.md |
| 17a | **PRICE_CEILING 0.980 -> 0.987** | **PROPOSED, not established** | +5 pts of count, +$1.90/day at size 20, inside its MDE; sign flips between a 1% and 2% loss rate on a population never traded live. Above 0.987 is INERT (the EV floor binds). Needs a 1-contract live count. |
| 17b | **TAU_MAX 30 -> 45/60** | **KILL CONFIRMED on the rebuilt replay** | flip rate 2.2x at 31-45s, 3.7x at 46-60s, and dearer; TAU_MAX 60 loses $316 on closes we already trade by burning slots. |
| 17c | **MIN_FILL_FRAC 0.5 -> 0.25** | **KILLED - the only result clearing its MDE, and it is negative** | -$0.0255/close, t = -5.3 vs MDE $0.0095. |
 **UNTESTED, queued** | Offer exists on 9.6% of confident markets, 51.8% of confident closes. |
| 18b | **pinsim REBUILT (seq-ordered book, event-driven, --gate-from)** | **DONE — 7 of 7 losses reproduced** | 210 real fills: exact price 33%→75%, bought under live gate 143→180, losses 1/9→9/9. Faithful 72h replay is WORSE ($31.52→$10.45 ceiling) and now agrees with live. Hedge re-read: 0.80 > 0.90; deployed. |
